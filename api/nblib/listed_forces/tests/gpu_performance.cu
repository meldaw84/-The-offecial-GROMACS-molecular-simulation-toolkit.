...
int main(int argc, char* argv[])
{
    ...
    {
        gmx::ListedForcesGpu gmxCalc(*gmx_ffparams, elecScale, deviceInfo, deviceContext, deviceStream, wcycle.get());
        gmxCalc.setPbc(PbcType::Xyz, box.legacyMatrix(), true);
	NBAtomDataGpu atomData;
        atomData.xq = reinterpret_cast<Float4*>(xqDevice);
        atomData.f  = forceDevice;
        atomData.fShift = fshiftDevice;
        gmxCalc.updateInteractionListsAndDeviceBuffers(noReorder, *idef, &atomData);
