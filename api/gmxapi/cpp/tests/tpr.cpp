...
TEST_F(GmxApiTest, DataLayoutAssumptions)
{
    // Check that PaddedVector<RVec> can be flattened safely.
    gmx::PaddedHostVector<gmx::RVec> paddedVector = { { 0., 0., 0. }, { 0., 0., 0. }, { 0., 0., 0. } };
    ASSERT_TRUE(paddedVector.size() == 3);
    static_assert(std::is_same_v<real*, std::decay_t<decltype(paddedVector[2].as_vec())>>);
    ASSERT_TRUE(static_cast<real*>(paddedVector[2].as_vec()) == paddedVector.data()->as_vec() + 6);
    ASSERT_TRUE(&paddedVector[2][0] == reinterpret_cast<real*>(paddedVector.data()) + 6);
}
...
TEST_F(GmxApiTest, TprReader)
{
    ...
    auto coordinatesBuffer = gmxapicompat::positions(*structureSource, real());
...
TEST_F(GmxApiTest, TprWriter)
{
...

    {
        ...
        auto inputCoordinatesBuffer = gmxapicompat::positions(*structureSource, real());
        ...
        // Trivially modify coordinates by adding 1.0 to all values.
        ...
        gmxapicompat::BufferDescription outputCoordinatesBuffer{ inputCoordinatesBuffer };
        outputCoordinatesBuffer.ptr = outputCoordinates.data();

        // Build the indexer from the buffer description.
        const std::function<real(size_t, size_t)> accessor = [outputCoordinatesBuffer](
                                                                     size_t i, size_t j) -> real {
            const auto* const base = static_cast<const real*>(outputCoordinatesBuffer.ptr);
            const auto* const ptr = base + (i * (outputCoordinatesBuffer.strides[0] / sizeof(real)))
                                    + (j * (outputCoordinatesBuffer.strides[1] / sizeof(real)));
            return *ptr;
        };

        auto tprBuilder = gmxapicompat::editTprFile(inputFilename);
        tprBuilder->positions(accessor);
...

    {
        ...
        auto  outputCoordinatesBuffer = gmxapicompat::positions(*outputStructureSource, real());
