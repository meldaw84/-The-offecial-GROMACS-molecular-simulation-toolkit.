...
//
// Reduce the number of atomic clashes by max a factor of 3 as consecutive threads add
// different components at the same time.
static inline void staggeredAtomicAddForce(sycl::global_ptr<Float3> gm_f,
                                           Float3                      f,
                                           const int                   localId)
{
                                           ...
                                           const int                   localId)
{
    ...
    atomicFetchAdd(gm_f[idx][offset[0]], f[0]);
    atomicFetchAdd(gm_f[idx][offset[1]], f[1]);
    atomicFetchAdd(gm_f[idx][offset[2]], f[2]);
}
