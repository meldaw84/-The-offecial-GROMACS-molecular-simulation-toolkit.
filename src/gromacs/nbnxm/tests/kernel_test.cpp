 ...
 *
 * The only things currently not covered are:
 * - LJ-PME with Lorentz-Berthelot combination rule, as this is only implemented
 *   in the plain-C reference kernel and currently the reference data is
 *   generated by the SIMD kernels.
 * - The Energy group pair energy output kernels.
 *
...
//! Test fixture
class KernelTest : public ::testing::TestWithParam<KernelInputParameters>
{
public:
    KernelTest() = default;
...
public:
    ...
    void TestBody() override
    {
        ...
        for (int i = 0; i < ssize(forcesOnly); i++)
        {
            for (int d = 0; d < DIM; d++)
            {
                EXPECT_REAL_EQ_TOL(forcesOnly[i][d], forces[i][d], forcesOnlyTolerance);
            }
        }
    }
};
