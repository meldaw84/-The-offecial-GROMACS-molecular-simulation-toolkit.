    ...
    Vc({}, { pinningPolicy })
{
    ...
    {
        int cj_size = Nbnxm::c_jClusterSize(kernelType);
        int numElements =
...
nbnxn_atomdata_t::SimdMasks::SimdMasks(const Nbnxm::KernelType kernelType)
{
#if GMX_SIMD
     ...
     */
    const int simd_4xn_diag_size = std::max(Nbnxm::c_iClusterSize(kernelType), simd_width);
                                       ...
                                       int                       n_energygroups)
{
    ...
    params->iClusterSize = Nbnxm::c_iClusterSize(kernelType);
}
    ...
    bUseBufferFlags(FALSE)
{
    ...
    if (simple)
    {
        if (bSIMD)
        {
            int pack_x = std::max(Nbnxm::c_iClusterSize(kernelType), Nbnxm::c_jClusterSize(kernelType));
            switch (pack_x)
            {
