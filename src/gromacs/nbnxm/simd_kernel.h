 ...
 * currently generated by a python script, not at build time, but any time
                     ...
                     nbnxn_atomdata_output_t*   out)
{
    ...
    const DiagonalMasker<nR, kernelLayout, getDiagonalMaskType<c_iClusterSize, c_jClusterSize>()> diagonalMasker(
            nbat->simdMasks);
    ...
    for (const nbnxn_ci_t& ciEntry : nbl->ci)
    {
        ...
        if constexpr (calculateEnergies)
        {
            ...
            if (do_self && l_cj[ciEntry.cj_ind_start].cj == cjFromCi<c_iClusterSize, c_jClusterSize>(ci_sh))
            {
                if (do_coul)
                {
