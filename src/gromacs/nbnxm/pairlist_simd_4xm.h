...
static constexpr int c_xStride4xN() { return std::max(GMX_SIMD_REAL_WIDTH, c_iClusterSize(kernelLayout)); }
                                        ...
                                        NbnxnPairlistCpuWork* work)
{
    ...
    {
        store(x_ci_simd + (3 * i + 0) * GMX_SIMD_REAL_WIDTH, SimdReal(x[ia + 0 * c_xStride4xN<kernelLayout>() + i] + shx));
        store(x_ci_simd + (3 * i + 1) * GMX_SIMD_REAL_WIDTH, SimdReal(x[ia + 1 * c_xStride4xN<kernelLayout>() + i] + shy));
        store(x_ci_simd + (3 * i + 2) * GMX_SIMD_REAL_WIDTH, SimdReal(x[ia + 2 * c_xStride4xN<kernelLayout>() + i] + shz));
    }
}
