                    ...
                    BoundingBox gmx_unused* bb_work_aligned)
{
    ...
    if (nbat->XFormat == nbatX4)
    {
        ...
        {
            GMX_ASSERT(geometry_.numAtomsJCluster == GMX_SIMD_REAL_WIDTH,
                       "With i-clusters double the size of j-clusters, we expect the j-cluster "
                       "size to match the SIMD width");
            calc_bounding_box_x_x4_halves<GMX_SIMD_REAL_WIDTH>(
                    numAtoms,
...
#endif
        {
            calc_bounding_box_packed<c_packX4>(
...
#endif
        {
            calc_bounding_box_packed<c_packX8>(
