    ...
    if constexpr (calculateLJInteractions)
    {
        ...
        if constexpr (ljCombinationRule != LJCombinationRule::LorentzBerthelot)
        {
            ...
            if constexpr (ljCombinationRule == LJCombinationRule::None)
            {
                ...
                {
                    if constexpr (kernelLayout == KernelLayout::r4xM || kernelLayout == KernelLayout::r8xM)
                    {
...
#    if UNROLLJ * 2 == UNROLLI
            ...
            {
                GMX_RELEASE_ASSERT(false, "Here we are currently accessing 64 bits, but 32 are stored");
                const int egps_j = nbatParams.energrp[cj >> 1];
                egp_jj[jj]        = ((egps_j >> ((jj * 2 + (cj & 1)) * egps_jshift)) & egps_jmask) * egps_jstride;
            }
#    else
