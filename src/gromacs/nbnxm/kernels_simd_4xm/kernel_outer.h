...

{
    ...
    constexpr int c_emptyCArraySize = 0;
#else
    constexpr int                     c_emptyCArraySize    = 1;
#endif
...
#ifdef CALC_COUL_TAB
#    ifdef TAB_FDV0
    constexpr KernelCoulombType coulombType = KernelCoulombType::EwaldTabulatedFDV0;
#    else
    constexpr KernelCoulombType coulombType = KernelCoulombType::EwaldTabulatedFAndV;
#    endif
#endif
#ifdef LJ_COMB_GEOM
...
#    endif
    ...
    SimdBool diagonalMaskV[UNROLLI == UNROLLJ ? nR : c_emptyCArraySize];
    SimdBool diagonalMask0V[UNROLLI == UNROLLJ ? c_emptyCArraySize : nR];
    SimdBool diagonalMask1V[UNROLLI == UNROLLJ ? c_emptyCArraySize : nR];
    ...
    SimdReal coulombParam1;
    SimdReal coulombParam2;
    if constexpr (coulombType == KernelCoulombType::RF)
    {
        /* Reaction-field constants */
        coulombParam1 = SimdReal(-2 * ic->reactionFieldCoefficient);
        coulombParam2 = SimdReal(ic->reactionFieldShift);
    }
    if constexpr (coulombType == KernelCoulombType::EwaldAnalytical)
    {
        /* The parameters are beta and beta^2 */
        coulombParam1 = SimdReal(ic->ewaldcoeff_q);
        coulombParam2 = SimdReal(ic->ewaldcoeff_q * ic->ewaldcoeff_q);
    }
    const real* tab_coul_F = nullptr;
    const real* tab_coul_V = nullptr;
    if constexpr (coulombType == KernelCoulombType::EwaldTabulatedFDV0
                  || coulombType == KernelCoulombType::EwaldTabulatedFAndV)
    {
        coulombParam1 = SimdReal(ic->coulombEwaldTables->scale);
        coulombParam2 = SimdReal(-0.5_real / ic->coulombEwaldTables->scale);
        if constexpr (coulombType == KernelCoulombType::EwaldTabulatedFDV0)
        {
    ...
    constexpr int nljsosp = numLJShiftOrSwitchParams<vdwModifier, calculateEnergies>();
    SimdReal      ljShiftOrSwitchParams[nljsosp > 0 ? nljsosp : c_emptyCArraySize];
    setLJShiftOrSwitchParameters<vdwModifier, calculateEnergies>(*ic, ljShiftOrSwitchParams);

    SimdReal ljEwaldParams[haveLJEwaldGeometric ? 5 : c_emptyCArraySize];
    ...
    SimdReal rc2_S = SimdReal(ic->rcoulomb * ic->rcoulomb);
    SimdReal rcvdw2_S;
    ...
    SimdReal minRsq_S = SimdReal(c_nbnxnMinDistanceSquared);
    ...
    for (const nbnxn_ci_t& ciEntry : nbl->ci)
    {
        ...
        real* vvdwtp[useEnergyGroups ? UNROLLI : c_emptyCArraySize];
        real* vctp[useEnergyGroups ? UNROLLI : c_emptyCArraySize];
        int   egps_i;
        if constexpr (useEnergyGroups)
        {
                ...
                if (do_self && l_cj[ciEntry.cj_ind_start].cj == (ci_sh << 1))
...
#endif
                    {
                        if (do_coul)
                        {
                            ...
                            if constexpr (coulombType == KernelCoulombType::RF)
                            {
                                Vc_sub_self = 0.5 * ic->reactionFieldShift;
                            }
                            if constexpr (coulombType == KernelCoulombType::EwaldTabulatedFDV0)
                            {
                                Vc_sub_self = 0.5 * tab_coul_F[2];
                            }
                            if constexpr (coulombType == KernelCoulombType::EwaldTabulatedFAndV)
                            {
        ...
        for (int i = 0; i < nR; i++)
        {
            ixV[i] = loadIAtomData<kernelLayout>(x, scix, i) + shX_S;
        }
        SimdReal iyV[nR];
        for (int i = 0; i < nR; i++)
        {
            iyV[i] = loadIAtomData<kernelLayout>(x, sciy, i) + shY_S;
        }
        SimdReal izV[nR];
        for (int i = 0; i < nR; i++)
        {
            izV[i] = loadIAtomData<kernelLayout>(x, sciz, i) + shZ_S;
        }

        SimdReal chargeIV[nR];
        if (do_coul)
        {
            for (int i = 0; i < nR; i++)
            {
                chargeIV[i] = facel * loadIAtomData<kernelLayout>(q, sci, i);
            }
        }

        constexpr bool c_ljCombLB = (ljCombinationRule == LJCombinationRule::LorentzBerthelot);
        SimdReal       halfSigmaIV[c_ljCombLB ? nR : c_emptyCArraySize];
        SimdReal       sqrtEpsilonIV[c_ljCombLB ? nR : c_emptyCArraySize];
        SimdReal       c6GeomV[(ljCombinationRule == LJCombinationRule::Geometric || haveLJEwaldGeometric) ? nR : c_emptyCArraySize];
        SimdReal c12GeomV[(ljCombinationRule == LJCombinationRule::Geometric) ? nR : c_emptyCArraySize];
        const real* nbfpV[(ljCombinationRule == LJCombinationRule::None) ? UNROLLI : c_emptyCArraySize];
        ...
        else
        {
            ...
            for (int i = 0; i < nR; i++)
            {
                nbfpV[i] = nbfp_ptr + type[sci + i] * numTypes * c_simdBestPairAlignment;
            }
        }
        if constexpr (haveLJEwaldGeometric)
        {
            /* We need the geometrically combined C6 for the PME grid correction */
            for (int i = 0; i < (half_LJ ? 2 : 4); i++)
            {
                c6GeomV[i] = loadIAtomData<kernelLayout>(ljc, sci2, i);
            }
        }
        ...
        /* Clear i atom forces */
        SimdReal forceIXV[nR];
        for (int i = 0; i < nR; i++)
        {
            forceIXV[i] = setZero();
        }
        SimdReal forceIYV[nR];
        for (int i = 0; i < nR; i++)
        {
            forceIYV[i] = setZero();
        }
        SimdReal forceIZV[nR];
        for (int i = 0; i < nR; i++)
        {
            forceIZV[i] = setZero();
        }
