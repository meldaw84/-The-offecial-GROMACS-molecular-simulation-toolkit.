                                                  ...
                                                  int                range[])
{
    ...
    const int iCell  = range[1] - 1;
    cell_size[iCell] = rowCoordinator->cellFrac[iCell + 1] - rowCoordinator->cellFrac[iCell];
    /* For this check we should not use DD_CELL_MARGIN,
     * but a slightly smaller factor,
     * since rounding could get use below the limit.
     */
    if (dimHasPbc && cell_size[iCell] < cellsize_limit_f * DD_CELL_MARGIN2 / DD_CELL_MARGIN)
    {
        char buf[22];
        gmx_fatal(FARGS,
