...
EnergyTerm::EnergyTerm(unsigned int findex, bool bStoreData, const std::string& eTerm, const std::string& eUnit) :
    eTerm_(eTerm),
    eUnit_(eUnit),
    nesum_(0),
    step0_(0),
    ...
    isEner_(false)
{
    ...
    {
        isEner_ = isEner_ || (gmx_strcasecmp_min(interaction_function[j].longname, eTerm.c_str()) == 0);
    }
}

void EnergyTerm::addFrame(double t, int64_t step, int nsum, double esum, double evar, double e)
{
    if (!firstFrameRead_)
    {
        t0_             = t;
...
EnergyAnalysisFrameIterator EnergyTerm::findFrame(int64_t iframe) const
{
    if (storeData())
    {
        ...
        else if (iframe != numFrames())
        {
            ...
            fprintf(stderr,
                    "WARNING: step %s out of range (0 <= step < %s)\n",
                    gmx_step_str(iframe, buf1),
...
bool EnergyTerm::errorEstimate(unsigned int nb, real* ee) const
{
    ...
    for (unsigned int b = 0; (b < nb); b++)
    {
        ...
        EnergyAnalysisFrameIterator f0  = findFrame(b * numFrames() / nb);
        EnergyAnalysisFrameIterator f1  = findFrame((b + 1) * numFrames() / nb);
        double                      sum = 0;
        int64_t                     np  = 0;
