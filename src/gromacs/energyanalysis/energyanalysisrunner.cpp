...
RunnerModule::RunnerModule(IEnergyAnalysisPointer module) :
...

{
    ...
    output_env_init(&oenv_,
                    gmx::getProgramContext(),
                    TimeUnit::Picoseconds, // Make sure the defaults match.
                    bView_,
                    plotSettings_.plotFormat(),
                    bVerbose_ ? 1 : 0);
}
...
void RunnerModule::initOptions(IOptionsContainer* options, ICommandLineOptionsModuleSettings* settings)
{
    ...
    options->addOption(DoubleOption("b").store(&t0_).timeValue().description(
            "First frame (%t) to read from energy file"));
    options->addOption(DoubleOption("e").store(&t1_).timeValue().description(
            "Last frame (%t) to read from energy file"));
    options->addOption(DoubleOption("dt").store(&tDelta_).timeValue().description(
            "Only use frame if t MOD dt == first time (%t)"));
    options->addOption(
            BooleanOption("w").store(&bView_).description("View output [TT].xvg[tt], [TT].xpm[tt], "
                                                          "[TT].eps[tt] and [TT].pdb[tt] files"));
    options->addOption(BooleanOption("v").store(&bVerbose_).description("Verbose output"));
...
// static
void EnergyAnalysisRunner::registerModule(CommandLineModuleManager*  manager,
                                          const char*                name,
...
// static
std::unique_ptr<ICommandLineOptionsModule> EnergyAnalysisRunner::createModule(IEnergyAnalysisPointer module)
{
