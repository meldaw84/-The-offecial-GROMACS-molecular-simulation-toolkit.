 ...
 */
static std::tuple<real, real, real> getResolutions(const gmx_mtop_t& mtop,
                                                   const bool        setMassesToOne,
                                                   const bool        useFep)
{
    // Class for computing the RMS and max(abs()) of a list of values
    class RmsMax
    {
    public:
        // Add \p count values
    ...
    real invMassResolution = std::max(invMassRmsMax.absMax() / std::numeric_limits<int16_t>::max(),
                                      invMassRmsMax.rms() * c_resolution);

    real chargeResolution = std::max(chargeRmsMax.absMax() / std::numeric_limits<int16_t>::max(),
                                     chargeRmsMax.rms() * c_resolution);
...
static std::vector<VerletbufAtomtype> getVerletBufferAtomtypes(const gmx_mtop_t& mtop,
                                                               ...
                                                               const bool        useFep)
{
    // Get the resolution for for 1/mass, charge and constraint length
    real invMassResolution;
    real chargeResolution;
    real constraintLengthResolution;

    std::tie(invMassResolution, chargeResolution, constraintLengthResolution) =
            getResolutions(mtop, setMassesToOne, useFep);
    ...
    for (const gmx_molblock_t& molblock : mtop.molblock)
    {
        ...
        std::vector<AtomNonbondedAndKineticProperties> prop(
                atoms->nr,
                AtomNonbondedAndKineticProperties(
                        invMassResolution, chargeResolution, constraintLengthResolution));
