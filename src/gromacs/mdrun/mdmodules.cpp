...
public:
    Impl() :
        ...
        lambdaDynamics_(LambdaDynamicsModuleInfo::create()),
    ...
    void makeModuleOptions(Options* options) const
    {
        ...
        lambdaDynamics_->mdpOptionProvider()->initMdpOptions(&appliedForcesOptions);
    ...
    std::unique_ptr<IMDModule>      lambdaDynamics_;
...
void MDModules::initMdpTransform(IKeyValueTreeTransformRules* rules)
{
    ...
    impl_->lambdaDynamics_->mdpOptionProvider()->initMdpTransform(appliedForcesScope.rules());
}
...
void MDModules::buildMdpOutput(KeyValueTreeObjectBuilder* builder)
{
    ...
    impl_->lambdaDynamics_->mdpOptionProvider()->buildMdpOutput(builder);
}
...
ForceProviders* MDModules::initForceProviders()
{
    ...
    impl_->lambdaDynamics_->initForceProviders(impl_->forceProviders_.get());
...
void MDModules::subscribeToPreProcessingNotifications()
{
    ...
    impl_->lambdaDynamics_->subscribeToPreProcessingNotifications(&impl_->notifiers_);
}
...
void MDModules::subscribeToSimulationSetupNotifications()
{
    ...
    impl_->lambdaDynamics_->subscribeToSimulationSetupNotifications(&impl_->notifiers_);
}
