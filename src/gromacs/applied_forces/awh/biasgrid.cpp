                       ...
                       const std::string& correctFormatMessage)
{
    ...
    for (int d = 0; d < grid.numDimensions(); d++)
    {
        ...
        {
            axis_.emplace_back(dataView[d][0], dataView[d][numDataPoints - 1], 0, numPoints[d], true, false);
        }
        else
        {
            axis_.emplace_back(
                    dataView[d][0], dataView[d][numDataPoints - 1], grid.axis(d).period(), numPoints[d], false, false);
        }
    }
