                        ...
                        gmx::ArrayRef<int>       covered)
{
    ...
    else
    {
        ...
        int notVisitedHighBoundary = firstNotVisited;
        int notVisitedLowBoundary  = period > 0 ? (lastNotVisited - period) : -(coverRadius + 1);

        int notCoveredLow  = notVisitedLowBoundary + coverRadius;
        int notCoveredHigh = notVisitedHighBoundary - coverRadius;

        for (int i = 0; i <= notVisitedHighBoundary; i++)
        {
        ...
        notVisitedHighBoundary = period > 0 ? (firstNotVisited + period) : (numPoints + coverRadius);
        notVisitedLowBoundary  = lastNotVisited;

        notCoveredLow  = notVisitedLowBoundary + coverRadius;
        notCoveredHigh = notVisitedHighBoundary - coverRadius;

        for (int i = notVisitedLowBoundary; i <= numPoints - 1; i++)
        {
