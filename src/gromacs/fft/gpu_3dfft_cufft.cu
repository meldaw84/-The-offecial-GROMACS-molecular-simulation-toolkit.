...
Gpu3dFft::ImplCuFft::ImplCuFft(bool allocateGrids,
    ...
    stream_(pmeStream),
    _gridSizesInX(gridSizesInXForEachRank.begin(), gridSizesInXForEachRank.end())
{
    ...
    GMX_RELEASE_ASSERT(numDomainsY == 1, "GPU PME-decomposition in y-dim not supported");

    bool useDecomposition = numDomainsX > 1;
...
#    endif
    ...
    if (useDecomposition)
    {
        allocateDeviceBuffer(&complexGrid2_,
                            complexGridSizePaddedTotal,
                            context);
        ...
        localx = gridSizesInXForEachRank[mpiRank];
        ...
        for (int i = 0; i < mpiSize; i++)
        {
            int ix            = gridSizesInXForEachRank[i];
            ...
            if (ymax < iy)
                ymax = iy;
            sendCount_[i] = complexGridSizePadded[ZZ] * localx * iy;
            recvCount_[i] = complexGridSizePadded[ZZ] * localy * ix;
        ...
        copyToDeviceBuffer(
                &d_s2g0x_, gridOffsetsInX.data(), 0, (mpiSize + 1), stream_, GpuApiCallBehavior::Sync, nullptr);
        copyToDeviceBuffer(
...
#    endif
        ...
        rank   = 1;
        batch  = localy * complexGridSizePadded[ZZ];
        result = cufftPlanMany(&planC2C1D_,
                               rank,
                               &complexGridSizePadded[XX], // 1D C2C part of the R2C
...
Gpu3dFft::ImplCuFft::~ImplCuFft()
{
    ...
    else
    {
        ...
        // Free fourier grid
        freeDeviceBuffer(&complexGrid2_);
    ...
    else
    {
        ...
        if (dir == GMX_FFT_REAL_TO_COMPLEX)
        {
            ...
            transposeXyzToYzx<true>(
                    complexGrid_, complexGrid2_, localx, complexGridSizePadded_[YY], complexGridSizePadded_[ZZ], stream_);
            stream_.synchronize();
...
#    endif
            ...
            convertBlockedYzxToYzx(complexGrid_,
                                   complexGrid2_,
                                   complexGridSizePadded_[XX],
                                   localy,
                                   complexGridSizePadded_[ZZ],
                                   d_xBlockSizes_,
                                   d_s2g0x_,
                                   mpiSize_,
                                   xMax_,
        ...
        else
        {
            ...
            transposeXyzToYzx<false>(
                    complexGrid2_, complexGrid_, complexGridSizePadded_[XX], localy, complexGridSizePadded_[ZZ], stream_);
