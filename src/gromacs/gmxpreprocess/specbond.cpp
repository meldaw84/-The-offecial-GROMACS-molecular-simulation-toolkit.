...
#define MAXCOL 7
            ...
            for (int b = 0; (b < nspec); b += MAXCOL)
            {
                ...
                for (int i = b; (i < e); i++)
                {
                    std::string bufAtom = gmx::formatString(
                            "%s%d", *pdba->atomname[specialBondAtomIdxs[i]], specialBondAtomIdxs[i] + 1);
                    fprintf(stderr, "%8s", bufAtom.c_str());
                }
                fprintf(stderr, "\n");
                /* print matrix */
                ...
                {
                    std::string bufMat = gmx::formatString(
                            "%s%d",
                            *pdba->resinfo[pdba->atom[specialBondAtomIdxs[i]].resind].name,
                            pdba->resinfo[specialBondResIdxs[i]].nr);
                    fprintf(stderr, "%8s", bufMat.c_str());
                    bufMat = gmx::formatString(
                            "%s%d", *pdba->atomname[specialBondAtomIdxs[i]], specialBondAtomIdxs[i] + 1);
                    fprintf(stderr, "%8s", bufMat.c_str());
                    int e2 = std::min(i, e);
                    for (int j = b; (j < e2); j++)
