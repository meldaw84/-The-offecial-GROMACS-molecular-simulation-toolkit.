...
TEST(XvgTests, CheckMissing)
{
    ...
    {
        const char* const inputMissing[] = { "0     2905.86    -410.199",
                                             "0.2     6656.67    -430.437",
                                             "0.4     5262.44    -409.399" };
        // Now check with missing data
        TestReferenceData      data(ReferenceDataMode::Compare);
        TestReferenceChecker   checker(data.rootChecker());
        gmx::StringInputStream sis(inputMissing);
...
TEST(XvgTests, CheckExtra)
{
    ...
    {
        const char* const inputExtra[] = {
            "0     2905.86    -410.199",   "0.2     6656.67    -430.437",
            "0.4     5262.44    -409.399", "0.6     5994.69    -405.763",
            "0.8     5941.37    -408.337", "1     5869.87    -411.124",
            "1.2     5889.87    -413.124"
        };
        // Now check with extra data
        TestReferenceData      data(ReferenceDataMode::Compare);
        TestReferenceChecker   checker(data.rootChecker());
        gmx::StringInputStream sis(inputExtra);
...
TEST(XvgTests, ReadIncorrect)
{
    ...
    {
        const char* const inputIncorrect[] = {
            "0     2905.86    -410.199",   "0.2     6656.67    -430.437",
            "0.4     5262.44    -409.399", "0.6     5994.69    -405.763",
        ...
        TestReferenceData      data(ReferenceDataMode::Compare);
        TestReferenceChecker   checker(data.rootChecker());
        gmx::StringInputStream sis(inputIncorrect);
