...
/*! \brief Function defined in a module's test code that uses the
 ...
 *   void registerMpiTests(int numRanks);
 *
 * that should call the tryToRegisterTest method of this class for
 * each such test case class, passing the number of ranks present at
 * run time, as well as the name of the test suite as well as type and
 * name of the test-case class. Note that this function must be
 * defined in the ::gmx::test namespace.
 */
...
public:
     ...
     * \param[in] numRanks       The number of MPI ranks in use at run time
